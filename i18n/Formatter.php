...
class Formatter extends \yii\i18n\Formatter
{
    ...
    public function asDate($value, $format = null)
    {
        ...
        if (Yii::$app->getI18n()->getIsIntlLoaded()) {
    ...
    public function asTimestamp($value, $format = null)
    {
        ...
        if (DateTimeHelper::isToday($timestamp)) {
            return $hasTimeInfo ? $this->asTime($timestamp, $format) : __('core', 'Today');
        }

        // If it was yesterday, display 'Yesterday'
        if (DateTimeHelper::isYesterday($timestamp)) {
            return __('core', 'Yesterday');
        }
        ...
        if (DateTimeHelper::isWithinLast($timestamp, '7 days')) {
            ...
            Yii::$app->getI18n()->getLocaleById($this->locale)->getWeekDayName($day);
        }
