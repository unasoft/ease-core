...
use Yii;
use ej\i18n\Locale;
use ej\dates\DateTime;
use ej\dates\DateInterval;
...
class DateTimeHelper
{
    ...
    public static function toDateTime($value, $assumeSystemTimeZone = false, $setToSystemTimeZone = true)
    {
        ...
        $defaultTimeZone = ($assumeSystemTimeZone ? \Yii::$app->getTimeZone() : 'UTC');
    ...
    public static function secondsToHumanTimeDuration($seconds, $showSeconds = true)
    {
        ...
        if ($weeks) {
            $timeComponents[] = $weeks.' '.($weeks == 1 ? Yii::t('app',
                    'week') : Yii::t('app', 'weeks'));
        }

        if ($days) {
            $timeComponents[] = $days.' '.($days == 1 ? Yii::t('app',
                    'day') : Yii::t('app', 'days'));
        }

        if ($hours) {
            $timeComponents[] = $hours.' '.($hours == 1 ? Yii::t('app',
                    'hour') : Yii::t('app', 'hours'));
        }

        if ($minutes || (!$showSeconds && !$weeks && !$days && !$hours)) {
            $timeComponents[] = $minutes.' '.($minutes == 1 ? Yii::t('app',
                    'minute') : Yii::t('app', 'minutes'));
        }

        if ($seconds || ($showSeconds && !$weeks && !$days && !$hours && !$minutes)) {
            $timeComponents[] = $seconds.' '.($seconds == 1 ? Yii::t('app',
                    'second') : Yii::t('app', 'seconds'));
        }
    ...
    private static function _getDateTranslations($language)
    {
        if (!isset(static::$_translationPairs[$language])) {
            if (strncmp(\Yii::$app->language, 'en', 2) === 0) {
                $sourceLocale = \Yii::$app->getLocale();
            } else {
                $sourceLocale = \Yii::$app->getI18n()->getLocaleById('en-US');
            }

            $targetLocale = \Yii::$app->getI18n()->getLocaleById($language);
