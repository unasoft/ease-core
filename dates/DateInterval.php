...
class DateInterval extends \DateInterval
{
    ...
    public function humanDuration($showSeconds = true)
    {
        ...
        if ($this->y) {
            $timeComponents[] = $this->y . ' ' . ($this->y > 1 ? __('core', 'years') : __('core', 'year'));
        }

        if ($this->m) {
            $timeComponents[] = $this->m . ' ' . ($this->m > 1 ? __('core', 'months') : __('core', 'month'));
        }

        if ($this->d) {
            $timeComponents[] = $this->d . ' ' . ($this->d > 1 ? __('core', 'days') : __('core', 'day'));
        }

        if ($this->h) {
            $timeComponents[] = $this->h . ' ' . ($this->h > 1 ? __('core', 'hours') : __('core', 'hour'));
        }
        ...
        if (!$showSeconds) {
            if ($minutes && round($this->s / 60)) {
                ...
                return __('core', 'less than a minute');
            }
        }

        if ($minutes) {
            $timeComponents[] = $minutes . ' ' . ($minutes > 1 ? __('core', 'minutes') : __('core', 'minute'));
        }

        if ($showSeconds && $this->s) {
            $timeComponents[] = $this->s . ' ' . ($this->s > 1 ? __('core', 'seconds') : __('core', 'second'));
        }
